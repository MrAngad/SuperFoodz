...
describe("Token Testing", function () {
  async function deployToken() {
    const [owner, addr1, addr2, addr3, _] = await ethers.getSigners();
    ...
    const token = await Token.connect(deployer).deploy(`${name}`, `${symbol}`, 300000000, 3141592654);
    await token.deployed();
    
    return { token, deployer, owner, addr1, addr2, addr3 };
  }
  ...
  describe("Base setup", async () => {
    ...
    it('Should set the right name', async () => {
      const { token, deployer, owner, addr1, addr2, addr3 } = await loadFixture(deployToken);
    ...
    it("Should set right Owner free tokens", async () => {
      const { token, deployer, owner, addr1, addr2, addr3 } = await loadFixture(deployToken);
      expect(await token.getFreeTokens(DEPLOYER_WALLET) / 10 ** 18).to.equal(initialSupply);
    })

    it("should set the right owner free balance", async () => {
      const { token, deployer, owner, addr1, addr2, addr3 } = await loadFixture(deployToken);
      expect(await token.getFreeTokens(DEPLOYER_WALLET)).to.equal(await token.balanceOf(DEPLOYER_WALLET));
    })
  });
  ...
  describe("Owner Transfers", async () => {
    ...
    it("Should allow owner to send free tokens", async () => {
      const { token, deployer, owner, addr1, addr2, addr3 } = await loadFixture(deployToken);
      await token.connect(deployer).transfer(addr1.address, 100);
      // expect(await token.balanceOf(DEPLOYER_WALLET)).to.equal((initialSupply+DECIMAL_ZEROS) - 100);
